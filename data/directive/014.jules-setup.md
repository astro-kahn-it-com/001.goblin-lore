...
* **IMMUTABLE RUNNER BOUNDARY:** You are strictly forbidden from modifying any file inside `apps/995.library/`[cite: 1].
* **WORKSPACE SCOPE:** You may ONLY add or modify files inside `packages/001.lore/`, `series/under-the-floorboards/`, and the scripts block of root `package.json`[cite: 1, 2].
* **NO UNAUTHORIZED PACKAGES:** Do not introduce Nx, Lerna, Yarn, or PNPM[cite: 1]. Use the monorepo's existing npm workspace configurations[cite: 1].
* **BYTE-ZERO FRONTMATTER LAW:** Every authored Markdown entity in `series/` must begin YAML frontmatter strictly at byte 0 (`---`) without leading whitespace or byte-order marks[cite: 8, 16].
* **DETERMINISTIC COMPILATION:** The compiler must enforce Unicode Normalization Form C (NFC), lexicographical key sorting, and integer basis point arithmetic[cite: 4, 25]. `Date.now()` is banned from compiler execution[cite: 25].
* **WINDOWS CMD.EXE SAFE ENCODING:** Avoid multi-byte unicode emojis that break Windows CLI shells; use standard ASCII telemetry tokens (`>>`, `[OK]`, `[FAIL]`, `::`)[cite: 2].
* **FAIL-SAFE EXIT:** If compilation or type validation fails, halt immediately and emit: `CONFLICT_BLOCKED`[cite: 2, 12].

---

### II. ARCHITECTURAL MISSION

Bootstrap the dedicated compile-time narrative legislative package **`001.lore`** (`@camp_candor/001.lore`) inside `packages/001.lore/`[cite: 2, 7]. This package serves as **The Law**: a deterministic three-pass compiler that validates hand-authored Markdown dossiers against strict Zod schemas and outputs a byte-identical `compiled/bible-state.json` stamped with a cryptographic SHA-256 state hash ($H_{\text{root}}$)[cite: 4, 10, 25].

In addition, wire `001.lore` into the Blessed terminal harness by implementing the unit/reducer pattern (`00.lore.unit`, `98.menu.unit`, `BEE.ts`, `hunt.ts`), allowing `apps/995.library/run.ts` to auto-discover and load the `LORE MENU` without modifying the immutable harness itself[cite: 1, 12].
...
## Wants (surface)
Spleen demands to be left alone by the warm pipe without having to submit to daily inspections.
...
## Surface Argument
Bog accuses Spleen of shaving candle wax from the emergency cache to lubricate his carving chisels.

## The Underneath
Both know that candle wax is the only fuel that burns without producing smoke visible through floor cracks.
...
3. `series/under-the-floorboards/compiled/bible-state.json` exists, is formatted in UTF-8 NFC, and contains a valid SHA-256 hash.
