...
`,
  ...
  it('Pass 2 compiles valid pardon and stamps COMPLIANT_BY_EXEMPTION in metadata', async () => {
    // Use real matter to parse and normalize the actual hash
    ...
    // We parse it then run it through the exact logic the compiler uses
    const { AgentSchema } = await import('../schemas/index.js')
    const parsedData = grayMatter(rawChar).data
    const charEntity = AgentSchema.parse(parsedData)
