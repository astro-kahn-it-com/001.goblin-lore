...
describe('01.series.unit Series Lifecycle', () => {
    ...
    it('creates a new series directory pre-populated with compliant examples', async () => {
        ...
        expect(
            fs.existsSync(path.join(testSeriesDir, 'series.config.json')),
        ).toBe(true)
        expect(
            fs.existsSync(
        ...
        expect(
            fs.existsSync(
                path.join(testSeriesDir, 'locations', 'central_hub.md'),
            ),
        ).toBe(true)
        expect(
            fs.existsSync(
    ...
    it('compiles the newly scaffolded series on demand via COMPILE_LORE', async () => {
        ...
        expect(
            fs.existsSync(path.join(testCompiledDir, 'bible-state.json')),
        ).toBe(true)
