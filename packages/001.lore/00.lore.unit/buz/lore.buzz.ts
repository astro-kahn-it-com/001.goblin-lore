...
export const compileLore = async (
    ...
    // if testing, point instanceDir to test/fixtures/ instead of series/
    const instanceDir =
        process.env.NODE_ENV === 'test' && seriesSlug.includes('test_series')
            ? path.resolve(process.cwd(), 'test/fixtures', seriesSlug)
            : path.resolve(repoRoot, 'series', seriesSlug)
