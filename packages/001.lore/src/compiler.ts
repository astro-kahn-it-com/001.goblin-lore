...
export function loadOntologyRules(ontologyDir?: string): Map<string, Rule> {
  let defaultRulesPath = path.resolve(
    process.cwd(),
    'packages/001.lore/schemas/ontology/rules.json',
  )
  if (!fs.existsSync(defaultRulesPath) && process.cwd().endsWith('packages/001.lore')) {
...
export function loadOntologyAuthorizers(
  ...
  let defaultAuthPath = path.resolve(
    process.cwd(),
    'packages/001.lore/schemas/ontology/authorizers.json',
  )
  if (!fs.existsSync(defaultAuthPath) && process.cwd().endsWith('packages/001.lore')) {
...
export function compileLoreInstance(options: CompileOptions): {
  ...
  if (!fs.existsSync(ontologyDir) && process.cwd().endsWith('packages/001.lore')) {
