...
test.serial('flattenUnit — flattens directory to root data/unit/<name>.txt without node_modules, dist, or data', async (t) => {
  // Find repo root
  let repoRoot = process.cwd();
  while (repoRoot && !(fs.existsSync(path.join(repoRoot, 'apps')) && fs.existsSync(path.join(repoRoot, 'packages')))) {
  ...
  await fs.writeFile(path.join(tempDir, 'node_modules', 'dummy', 'index.js'), 'export const leak = true;\n');
  await fs.writeFile(path.join(tempDir, 'dist', 'bundle.js'), 'export const compiled = true;\n');
  await fs.writeFile(path.join(tempDir, 'data', 'store.json'), '{"key":"value"}');
  ...
  t.true(relativeOutputPath.startsWith('data/unit/'), `Path must start with data/unit/, got: ${relativeOutputPath}`);
  t.true(relativeOutputPath.endsWith('.txt'), `Path must end with .txt, got: ${relativeOutputPath}`);

  const absoluteOutputFile = path.join(repoRoot, relativeOutputPath);
  t.true(fs.existsSync(absoluteOutputFile), `File should exist on disk at ${absoluteOutputFile}`);

  const content = await fs.readFile(absoluteOutputFile, 'utf8');
  t.true(content.includes('export const hello = "world";'), 'Should contain source code');
