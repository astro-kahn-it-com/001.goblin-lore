...
export const initServer = (cpy: ServerModel, bal: ServerBit, ste: State) => {
  ...
  (async () => {
    try {
      ...
      const [resserver, resSpace] = await Promise.all([fetch(urlserver), fetch(urlSpace)]);

      // 4. THE INTEGRITY CHECK
      if (!resserver.ok) throw new Error(`server fetch failed: ${resserver.status}`);
      if (!resSpace.ok) throw new Error(`Space fetch failed: ${resSpace.status}`);
      ...
      if (bal.slv != null) {
        bal.slv({
          intBit: {
            idx: 'init-server-err',
            dat: err instanceof Error ? err.message : 'Unknown Error',
          },
        });
      }
    }
...
export const visionServer = async (cpy: ServerModel, bal: ServerBit, ste: State) => {
...
export const intellectServer = async (cpy: ServerModel, bal: ServerBit, ste: State) => {
  const userPrompt = bal.src || 'No prompt provided to the mire.';
...
export type Actions = InitServer | UpdateServer | TestServer | IntellectServer | VisionServer;
...
describe('server', () => {
  ...
  it('should wake up the server if url is provided', async () => {
    ...
    const globalFetch = vi.spyOn(global, 'fetch').mockImplementation((url) => {
      if (url === urlserver) {
