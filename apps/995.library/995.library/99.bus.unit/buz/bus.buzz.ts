...
export const initBus = (cpy: BusModel, bal: BusBit, ste: State) => {
  ...
  ste.bus = (idx: string, dat: any, bit: any) => void updateBus(cpy, { idx, dat, bit }, ste);
      ...
      void ste
        ...
        .then((bit: any) => {
          ...
          client.on('message', (tpc: string, msg: any) => {
            void messageBus(cpy, { idx: tpc, src: msg, bit: a.idx }, ste);
          });
          client.on('connect', () => {
            // console.log(a.idx + ' connected ' + a.src);
            void openBus(cpy, { idx: 'init-bus', lst: cpy.actList, bit: a.idx }, ste);
            complete(lst);
          });
        });
    });
...
const patch = (ste: State, type: string, bale: any) => ste.dispatch({ type, bale });
