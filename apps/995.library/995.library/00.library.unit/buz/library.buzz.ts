...
export const listLibrary = (cpy: LibraryModel, bal: LibraryBit, ste: State) => {
  ...
  const IGNORE = new Set(['node_modules', '.git', 'dist', 'page', '$RECYCLE.BIN', 'Config.Msi', 'vision']);

  function hasDirectUnits(dir: string) {
    try {
      const entries = fs.readdirSync(dir, { withFileTypes: true });
      for (const entry of entries) {
        if (entry.isDirectory() && /^\d{2}\..+\.unit$/.test(entry.name)) {
          return true;
        }
      }
    } catch (e) {
...
export const updateLibrary = async (cpy: LibraryModel, bal: LibraryBit, ste: State) => {
  ...
  list.forEach(async (a, b) => {
    ...
    if (FS.lstatSync(checkPath).isDirectory()) {
      ...
      const unitImportSte = 'import ' + unitName + 'Unit from "' + unitImportSrc + '";';

      const faceImportSrc = './' + a + '/fce/' + element + '.interface';
      const faceImportSte = 'import ' + unitName + ' from "' + faceImportSrc + '";';

      const modlImportSrc = './' + a + '/' + element + '.model';
      const modlImportSte = 'import { ' + unitName + 'Model } from "' + modlImportSrc + '";';

      const redcImportSrc = './' + a + '/' + element + '.reduce';
      const redcImportSte = 'import * as reduceFrom' + unitName + ' from "' + redcImportSrc + '";';

      const reduced = element + ' : reduceFrom' + unitName + '.reducer';
      const model = element + ' : ' + unitName + ' = new ' + unitName + 'Model();';
...
export const progressLibrary = async (cpy: LibraryModel, bal: LibraryBit, ste: State) => {
  /**
  ...
  try {
    ...
    if (bal.slv) bal.slv({ libBit: { idx: 'progress-library', src: bal.src } });
  } catch (err) {
    await ste.hunt(ActCns.UPDATE_CONSOLE, {
      idx: 'cns00',
...
export const launchLibrary = async (cpy: LibraryModel, bal: LibraryBit, ste: State) => {
  const fs = require('fs');
  const path = require('path');
...
export const flatLibrary = async (cpy: LibraryModel, bal: LibraryBit, ste: State) => {
  const fs = require('fs-extra');
  const path = require('path');
  ...
  const isRepoRoot = (dir: string) => {
    try {
      return fs.existsSync(path.join(dir, 'apps')) && fs.existsSync(path.join(dir, 'packages')) && fs.existsSync(path.join(dir, 'package.json'));
    } catch {
      return false;
    }
  };
  ...
  const IGNORED_DIRS = new Set(['node_modules', 'dist', 'data', '.git', '.wrangler']);
  const CODE_EXTS = new Set(['.ts', '.tsx', '.js', '.cjs', '.mjs', '.jsonc', '.toml', '.yml', '.yaml']);
  const ALLOWED_FILES = new Set(['.gitignore', 'package.json', 'AGENTS.md', 'AGENT_INSTRUCTIONS.md']);
  ...
  try {
    const targetRoots = ['apps', 'packages', '.github', '.'].map((folder) => path.join(repoRoot, folder));
    ...
    for (const targetRoot of targetRoots) {
      ...
      if (targetRoot === repoRoot) {
        ...
        for (const entry of subEntries) {
          if (!entry.isDirectory()) continue;
          if (IGNORED_DIRS.has(entry.name) && entry.name !== 'schema') continue;
    ...
    const codeFiles = allScannedFiles.filter((file) => {
      ...
      const isAllowedFile = ALLOWED_FILES.has(filename) || filename.startsWith('tsconfig');
    ...
    const relOutput = path.relative(repoRoot, outputFile).replace(/\\/g, '/');
    if (ste) {
      await ste.hunt(ActCns.UPDATE_CONSOLE, {
        idx: 'cns00',
