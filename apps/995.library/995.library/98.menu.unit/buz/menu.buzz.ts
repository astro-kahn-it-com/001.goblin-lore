...
export const initMenu = async (cpy: MenuModel, bal: MenuBit, ste: State) => {
  ...
  if (!opened) {
    ...
    const libSubPath = path.resolve(process.cwd(), 'apps/995.library/package.json');
...
export const openMenu = async (cpy: MenuModel, bal: MenuBit, ste: State) => {
  ...
  const pkg = FS.existsSync(libPkgPath) ? FS.readJsonSync(libPkgPath) : { version: '0.0.0' };
...
export const updateMenu = async (cpy: MenuModel, bal: MenuBit, ste: State) => {
  ...
  bit = await ste.hunt(ActChc.OPEN_CHOICE, {
    dat: {
      ...
      clr1: Color.YELLOW,
      cb: (choice: string) => {
        const text = descriptions[choice] || cpy.menuRouteDescriptions?.get(choice) || 'No description available.';
        text.split('\n').forEach((src) =>
