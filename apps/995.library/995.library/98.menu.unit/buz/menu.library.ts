...
export const libraryMenu = async (cpy: MenuModel, bal: MenuBit, ste: State) => {
  ...
  lst = [ActGer.LORE_GEARS.split(']')[1], ActGer.CREATE_GEARS.split(']')[1], ActUnt.UPDATE_UNIT.split(']')[1], ActAct.UPDATE_ACTION.split(']')[1], ActUnt.CREATE_UNIT.split(']')[1], ActUnt.FLATTEN_UNIT.split(']')[1], ActLib.FLAT_LIBRARY.split(']')[1], ActLib.PROGRESS_LIBRARY.split(']')[1], ActLib.UPDATE_LIBRARY.split(']')[1], ActLib.LIST_LIBRARY.split(']')[1], ActLib.LAUNCH_LIBRARY.split(']')[1], 'ROOT MENU'];

  const descriptions: any = {
    [ActUnt.UPDATE_UNIT.split(']')[1]]: '-Update an existing unit\nin the library.',
    [ActAct.UPDATE_ACTION.split(']')[1]]: '-Update an action for\na specific unit \nso the library can use the action',
    [ActUnt.CREATE_UNIT.split(']')[1]]: '-Create a new unit\nwith a specified verb.',
    [ActUnt.FLATTEN_UNIT.split(']')[1]]: '-Flatten a unit into\nits constituent files.',
    [ActLib.FLAT_LIBRARY.split(']')[1]]: '-Flatten library code\ninto data/flat.',
    [ActLib.UPDATE_LIBRARY.split(']')[1]]: '-Regenerate the library\nwiring manifest (BEE.ts).',
    [ActLib.LIST_LIBRARY.split(']')[1]]: '-List all the units\ncurrently in the library.',
    [ActGer.LORE_GEARS.split(']')[1]]: '-List all GEARS units\ncurrently in the library.',
    [ActGer.CREATE_GEARS.split(']')[1]]: '-List all GEARS units\ncurrently in the library.',
  ...
  switch (src) {
    ...
    case ActUnt.UPDATE_UNIT.split(']')[1]:
      ...
      if (pivotSelection) pivotSelection = pivotSelection.replace(/[[\]]/g, '');
    ...
    case ActUnt.FLATTEN_UNIT.split(']')[1]:
      ...
      const IGNORE_MENU_DIRS = new Set(['node_modules', 'dist', 'data', '.git']);
      ...
      try {
        if (FS.existsSync(targetPath)) {
          ...
          for (const entry of entries) {
            if (entry.isDirectory() && !IGNORE_MENU_DIRS.has(entry.name)) {
    ...
    case ActLib.FLAT_LIBRARY.split(']')[1]:
      ...
      await ste.hunt(ActCns.UPDATE_CONSOLE, {
        idx: 'cns00',
        src: 'Library flattened to ' + (bit?.libBit?.src || 'data/flat'),
      });
      bit = await ste.hunt(ActMnu.PRINT_MENU, bit);
      break;
